...
% Load data
data = load("/Users/cristianvoltan/Desktop/unipd/tirocinio/CAD/pneumatic_solution/astana_P/dati_porta_pneumatica_astana_P.mat");

t = data.tempo(:);

door_sx = data.apertura_sx(:);
door_dx = data.apertura_dx(:);

vel_sx = data.velocita_sx(:);
vel_dx = data.velocita_dx(:);
...
quat_sx = data.quat_snodo_base_sx;
quat_dx = data.quat_snodo_base_dx;
...
plot(t, cyl_sx, "LineWidth", 3.0)
hold on
plot(t, cyl_dx, "LineWidth", 3.0)
grid on
...
% Figure - Pneumatic actuator force
...
% Figure - Mechanical actuation power
