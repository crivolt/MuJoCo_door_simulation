...
# LOAD MODEL
model = mujoco.MjModel.from_xml_path("/Users/cristianvoltan/Desktop/unipd/tirocinio/CAD/studio_54/studio_54.xml")
...
model.dof_frictionloss[door_dofadr] = 54.02
...
# HOME POSITION
q_home = np.array([0.0, -2.01, -0.597, -2.01, 0.0628, 0.0])
...
# TRAJECTORY
trajectory = np.load("/Users/cristianvoltan/Desktop/unipd/tirocinio/CAD/studio_54/traiettorie/traiettoria_porta.npy")

OPEN_DEG  = 62
T_OPEN    = 1.0
T_CLOSE   = 1.0

n_points_open = int(len(trajectory) * OPEN_DEG / 200)
...
# POLYNOMIAL PROFILE
# theta(s) = a0 + a1*s + a2*s^2 + a3*s^3,  s in [0, 1]
#
# Boundary conditions:
#   theta(0) = 0        -> starts at zero
#   theta(1) = theta_f   -> ends at target angle
#   omega(0) = 0        -> zero velocity at start
#   omega(1) = 0        -> zero velocity at end

def compute_cubic(theta_f=62.0):
    """Solve for cubic polynomial coefficients [a0, a1, a2, a3]."""
    A = np.array([
        [1, 0, 0, 0],   # theta(0) = 0
        [1, 1, 1, 1],   # theta(1) = theta_f
        [0, 1, 0, 0],   # omega(0) = 0
        [0, 1, 2, 3],   # omega(1) = 0
    ])
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    for step in range(MAX_STEPS):
        ...
        match phase:
            ...
            # Friction drops from high to low after 1° to model latch release.
            case 3:
                ...
                if door_angle_now < 1.0:
                    ...
                    model.dof_frictionloss[door_dofadr] = 5.402
            ...
            # same cubic velocity profile as the opening phase.
            case 5:
