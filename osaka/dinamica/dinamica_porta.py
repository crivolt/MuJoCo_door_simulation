...
model = mujoco.MjModel.from_xml_path("/Users/cristianvoltan/Desktop/unipd/tirocinio/CAD/osaka/osaka.xml")
...
door_joint_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, "slide_dx") # CAMBIARE SE VOGLIO LA DX
door_qposadr = model.jnt_qposadr[door_joint_id]

# sites

site_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, "grasp_site")
target_site_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, "target_manigliadx") # CAMBIARE SE VOGLIO LA DX
weld_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_EQUALITY, "presa_dx") # CAMBIARE SE VOGLIO LA DX
...
q_home = np.array([-0.927, -0.934, 0.0, -2.32, 0.0, joint6_angle, joint7_angle])
...
trajectory = np.load("/Users/cristianvoltan/Desktop/unipd/tirocinio/CAD/osaka/traiettorie/traiettoria_destra.npy")

OPEN_METERS = 0.32      
MAX_METERS  = 0.4      
n_points_open = int(len(trajectory) * OPEN_METERS / MAX_METERS)
...
close_index     = 0
TRAJ_WAIT       = 2 # steps before moving to next point
traj_wait_timer = 0
...
KP             = 1.0    
KI             = 0.15  
INTEGRAL_CLIP  = 0.08   # anti-windup
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    for step in range(30000):
        ...
        match phase:
            ...
            case 0:
                ...
                if step % ik_every == 0:
                    ...
                    if dist < 0.01 or stall_counter >= STALL_LIMIT:
                        reset_integral()
                        phase = CLOSE_GRIPPER
                        phase_timer = 0
            ...
            case 6:
                ...
                HOLD_STEPS = int(8.0 / model.opt.timestep)  # 3 secondi in steps
        ...
        if step % 50 == 0:
            door_pos_m = data.qpos[door_qposadr]
            time_sec = step * model.opt.timestep
            print(f"Tempo {time_sec:.2f} | fase {phase} | apertura porta: {door_pos_m:.4f} m")

    door_pos_m = data.qpos[door_qposadr]
    print(f"Apertura porta finale: {data.qpos[door_qposadr]:.4f} m")
