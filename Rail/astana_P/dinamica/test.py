...
# Default opening/closing impedance 
K_OPENING  = 700.0
D_OPENING  = 100.0
...
# NEUTRAL POSITION
Q_NEUTRAL = np.array([0.0, -1.57, -1.57, 0.0, 0.0, 0.0])
...
door_configs = {
    "sx": dict(
        ...
        q_home        = np.array([0.0, -1.88, -1.19, -1.57, 0.0, 0.0]),
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    for step in range(MAX_STEPS):
        ...
        match phase:
            ...
            # K, D and gain are taken from the active door config.
            case 4:
                set_joint_impedance(active["K_opening"], active["D_opening"], JOINT_FORCE_LIMITS)
                data.eq_active[weld_id] = 1
                ...
                door_angle_now = abs(np.rad2deg(data.qpos[door_qposadr]))
                if door_angle_now < 1.0:
                    model.dof_frictionloss[door_dofadr] = 22.85
                else:
                    model.dof_frictionloss[door_dofadr] = 4.57

                solve_position_ik(active["trajectory"][traj_index, 0:3], active["gain_open"])
            ...
            # K, D and gain are taken from the active door config.
            case 6:
                set_joint_impedance(active["K_closing"], active["D_closing"], JOINT_FORCE_LIMITS)
                data.eq_active[weld_id] = 1
                ...
                solve_position_ik(active["traj_close"][close_index, 0:3], active["gain_close"])
