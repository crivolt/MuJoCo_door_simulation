...
# FRICTION
FRICTION_STATIC  = 41.9
FRICTION_DYNAMIC = 15.0
...
SLITTA_VEL_OPEN  = 0.08
SLITTA_VEL_CLOSE = 0.06


# NEUTRAL POSITION
Q_NEUTRAL = np.array([0.0, -1.57, 1.32, -1.51, -1.51, 0.0])
...
# TRAJECTORIES
def build_traj_close(traj, open_m, max_m=1.06):
...
door_configs = {
    "sx": dict(
        ...
        q_home        = np.array([0.503, -1.57, 1.32, -1.51, -1.51, 0.0]),
    ...
    "dx": dict(
        ...
        q_home        = np.array([-0.942, -1.57, 1.32, -1.51, -1.51, 0.0]),
        site_name     = "attachment_site",
...
for key in ["slide_sx", "slide_dx"]:
    ...
    model.dof_frictionloss[model.jnt_dofadr[jid_]] = FRICTION_STATIC
        ...
        match phase:
            ...
            case 2:
                ...
                if phase_timer >= CLOSE_DURATION:
                    ...
                    print(f"[WELD] errore posizione pre-weld: {err_weld:.4f} m")
            ...
            # Friction drops from static to dynamic after 1 cm.
            case 4:
            ...
            case 6:
                ...
                if s >= 1.0:
                    print(f"[CLOSE] porta chiusa: {door_pos_now:.4f} m "
                          f"| frictionloss finale: "
                          f"{model.dof_frictionloss[door_dofadr]:.2f} N"
