...
# FRICTION AND MECHANICAL STEP PARAMETERS

FRICTION_STATIC  = 33.0    # [N] breakaway force (includes gasket compression peak)
FRICTION_DYNAMIC = 3.0     # [N] dynamic friction of rollers on rail

STEP_ZONE_OPEN  = 0.05    # [m] zone where friction drops from static to dynamic
STEP_ZONE_CLOSE = 0.05    # [m] step zone width on closing
STEP_PEAK_CLOSE = 20.0    # [N] peak friction at end stop on closing


def update_door_friction(door_pos: float, door_dofadr: int, closing: bool = False) -> None:
    if not closing:
        if door_pos < STEP_ZONE_OPEN:
            fl = FRICTION_STATIC
        else:
            fl = FRICTION_DYNAMIC
    else:
        if door_pos <= STEP_ZONE_CLOSE:
            alpha = 1.0 - (door_pos / STEP_ZONE_CLOSE)
            fl = FRICTION_DYNAMIC + (STEP_PEAK_CLOSE - FRICTION_DYNAMIC) * alpha
        else:
            fl = FRICTION_DYNAMIC

    model.dof_frictionloss[door_dofadr] = fl
...
all_joint_names = ["shoulder_pan_joint", "shoulder_lift_joint", "elbow_joint",
...
# NEUTRAL POSITION
Q_NEUTRAL = np.array([0.0, -1.76, 1.92, 0.314, 1.57, 0.0])
...
T_OPEN  = 1.0   # duration of door opening  [s]  ← tune as needed
T_CLOSE = 1.0   # duration of door closing  [s]  ← tune as needed
...
cubic_c = compute_cubic(theta_f=1.0)


# TRAJECTORIES

def build_traj_close(traj, open_m, max_m=0.8):
    n = max(2, min(int(len(traj) * open_m / max_m), len(traj)))
...
traj_close_sx, n_open_sx = build_traj_close(trajectory_sx, OPEN_M)
traj_close_dx, n_open_dx = build_traj_close(trajectory_dx, OPEN_M)
...
door_configs = {
    "sx": dict(
        ...
        q_home        = np.array([0.44, -1.26, 1.7, 0.314, 1.57, 0.0]),
    ...
    "dx": dict(
        ...
        q_home        = np.array([-0.565, -1.38, 1.85, 0.314, 1.57, 0.0]),
        site_name     = "attachment_site",
...
def setup_door(key):
    global active, site_id, target_site_id, weld_id
    global door_qposadr, door_dofadr
    ...
    door_dofadr  = model.jnt_dofadr[door_jid_]

    q_des_all[:] = Q_NEUTRAL.copy()
    reset_integral()

    update_door_friction(0.0, door_dofadr, closing=False)
...
for key in ["slide_sx", "slide_dx"]:
    jid_ = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, key)
    dof_ = model.jnt_dofadr[jid_]
    update_door_friction(0.0, dof_, closing=False)
...
MAX_STEPS = int(50000 * N_CYCLES * len(DOOR_SEQUENCE)) + 10000
    ...
    for step in range(MAX_STEPS):
        ...
        match phase:
            ...
            case 0:
                ...
                q_des_all[:] = interpolate_arm(q_interp_start, active["q_home"],
            ...
            case 1:
                ...
                if step % ik_every == 0:
                    ...
                    if dist < 0.0005 or stall_counter >= STALL_LIMIT:
                        print(f"[APPROACH] dist={dist:.4f}m | q_home={active['q_home']}")
            ...
            case 2:
                ...
                if phase_timer >= CLOSE_DURATION:
                    ...
                    print(f"[WELD] pre-weld position error: {err_weld:.4f} m")
            ...
            # Friction drops from static to dynamic after STEP_ZONE_OPEN.
            case 4:
                set_joint_impedance(K_TRACKING, D_TRACKING, JOINT_FORCE_LIMITS)
                data.eq_active[weld_id] = 1
                ...
                theta_norm = cubic_theta(s, cubic_c)   # 0 → 1
                traj_index = int(theta_norm * (active["n_open"] - 1))
                traj_index = np.clip(traj_index, 0, active["n_open"] - 1)

                door_pos_now = abs(data.qpos[door_qposadr])
                update_door_friction(door_pos_now, door_dofadr, closing=False)

                solve_position_ik(active["trajectory"][traj_index, 0:3], gain_opening)

                if s >= 1.0:
                    print(f"[OPEN] porta aperta: {door_pos_now:.3f} m "
                          f"| frictionloss: {model.dof_frictionloss[door_dofadr]:.2f} N")
                    reset_integral()
                    q_des_all[:] = data.qpos[all_qposadr].copy()
                    open_start_time = None
                    phase       = WAIT_OPEN
            ...
            case 5:
                ...
                if phase_timer >= WAIT_BEFORE_CLOSE_STEPS:
                    ...
                    # Build close trajectory from actual reached index
                    traj_close_now       = active["trajectory"][:traj_index+1][::-1].copy()
                    active["traj_close"] = traj_close_now
            ...
            case 6:
                ...
                door_pos_now = abs(data.qpos[door_qposadr])
                update_door_friction(door_pos_now, door_dofadr, closing=True)

                solve_position_ik(active["traj_close"][close_index, 0:3], gain_closing)

                if s >= 1.0:
                    print(f"[CLOSE] porta chiusa: {door_pos_now:.4f} m "
                          f"| frictionloss: {model.dof_frictionloss[door_dofadr]:.2f} N"
                          f"| vel={data.qvel[door_dofadr]:+.3f} m/s")
                    reset_integral()
                    q_des_all[:] = data.qpos[all_qposadr].copy()
                    close_start_time = None
                    phase       = HOLD_CLOSED
            ...
            case 8:
                ...
                q_des_all[:] = interpolate_arm(q_interp_start, Q_NEUTRAL,
                                               slitta_interp_start, SLITTA_NEUTRAL)
        ...
        if phase != DONE:
            ...
            log_door_m.append(data.qpos[door_qposadr])
            log_door_vel.append(data.qvel[door_dofadr])
            ...
            if step % 50 == 0:
                ...
                fl_now        = model.dof_frictionloss[door_dofadr]

                print(
                    f"t={t:6.2f}s | ciclo {cycle_count+1}/{N_CYCLES} "
                    f"| porta {lbl:>8s} | {STATE_NAMES[phase]}"
                    f"| slitta={spos:+.3f} | porta={door_pos:+.4f} m "
                    f"| vel={door_vel:+.3f} m/s "
                    f"| fl={fl_now:5.1f} N "
