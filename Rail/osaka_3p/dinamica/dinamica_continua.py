...
# ══════════════════════════════════════════════════════════════════
N_CYCLES = 2        # quante volte ripetere il ciclo  sx → dx
OPEN_DEG = 62       # apertura massima porta in gradi
...
ik_joint_names  = ["joint1","joint2","joint3","joint4","joint5","joint6","joint7"]
...
# ── State machine ─────────────────────────────────────────────────
MOVE_TO_DOOR    = 0   # neutrale → posizione porta  (slitta + braccio interpolati)
APPROACH_HANDLE = 1
CLOSE_GRIPPER   = 2
STABILIZE_WELD  = 3
OPEN_DOOR       = 4
...
CLOSE_DOOR      = 6
HOLD_CLOSED     = 7
RETURN_NEUTRAL  = 8   # posizione porta → neutrale  (slitta + braccio interpolati)
DONE            = 9
...
# interpolazione slitta/braccio
q_interp_start  = Q_NEUTRAL.copy()   # punto di partenza interpolazione
slitta_interp_start = SLITTA_NEUTRAL  # posizione slitta all'inizio del movimento
...
def setup_door(key):
    """Carica la configurazione della porta e aggiorna gli ID MuJoCo."""
...
def interpolate_arm(q_start, q_end, slitta_start, slitta_target):
    """
    Calcola q_des_all interpolando tra q_start e q_end in base
    al progresso fisico della slitta. Ritorna alpha [0,1].
    """
...
# ── Stato iniziale ────────────────────────────────────────────────
# Porta il robot alla posizione neutra
...
# Setup prima porta, salva punto partenza interpolazione
door_index  = 0
cycle_count = 0
    ...
    for step in range(MAX_STEPS):
        ...
        match phase:
            ...
            case 0:
                ...
                # braccio segue la slitta linearmente
                ...
                if abs(slitta_pos - slitta_tgt) < 0.01 and abs(slitta_vel) < 0.01:
                    ...
                    # aggiorna q_des_ik alla home corrente
            ...
            case 7:
                ...
                if phase_timer >= int(3.0 / model.opt.timestep):
                    ...
                    SLITTA_LOCKED = False
                    # salva punto di partenza per il ritorno al neutrale
            ...
            case 8:
                ...
                # braccio torna a Q_NEUTRAL mentre la slitta torna a 0
                ...
                if abs(slitta_pos - SLITTA_NEUTRAL) < 0.01 and abs(slitta_vel) < 0.01:
                    # arrivati al neutrale → prossima porta o ciclo
                    data.qpos[slitta_qposadr] = SLITTA_NEUTRAL
                    data.qvel[slitta_dofadr]  = 0.0
                    q_des_all[:] = Q_NEUTRAL.copy()
                    q_des_ik[:]  = Q_NEUTRAL.copy()
