...
# HOME POSITION
q_home = np.array([1.51, -2.2, 1.73, -1.63, -1.63, 0])
        ...
        match phase:
            ...
            case 3:
                ...
                if abs(door_angle_now) < 1.0:
                    model.dof_frictionloss[door_dofadr] = 9.30
                else:
                    model.dof_frictionloss[door_dofadr] = 1.86
            ...
            case 5:
                ...
                if s >= 1.0:
                    door_vel_final = np.rad2deg(data.qvel[door_dofadr])
                    reset_integral()
                    q_des_all[:] = data.qpos[all_qposadr].copy()
                    close_start_time = None
                    phase = HOLD_CLOSED
